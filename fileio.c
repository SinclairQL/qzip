...
#  ifndef NODIR
     DIR *opendir OF((char *));
#  endif /* !NODIR */
#  if !defined __WATCOMC__ && !defined CONVEX
...
#ifdef QDOS
int wild(w)
char *w;                /* path_pattern to match */
/* If not in exclude mode, expand the pattern based on the contents of the
   file system.  Return an error code in the ZE_ class. */
{
  char a[64];           /* device and file name */
  char p[5];            /* device path */
  qdirect qd;           /* directory entry */
  chanid_t chid;        /* channel ID */
  int f = 0;            /* true if there was a match */

  /* QDOS path starts with 5 char device name */
  if (strlen(w) == 5)
    return procname(w);   /* just device path */

  p[5] = 0;
  strncpy(p, w, 5);       /* device and file name */ 

  /* open device name */
  if ((chid = io_open(p, QDR_DIR)) > 0)
  {
    while (io_fstrg(chid, -1, &qd, sizeof(qd)) > 0)
    {
      strncat(strcpy(a, p), qd.d_name, qd.d_szname);
      if (MATCH(a, w))
        if (procname(a) == ZE_OK)
          f = 1; 
    }
  ...
  else
  {
    ...
    if ((s.st_mode & S_IFDIR) == 0)
    {
      /* add or remove name of file */
        ...
        if (recurse && ((chid = io_open(n, QDR_DIR)) > 0))
        {
          ...
          while (io_fstrg(chid, -1, &qd, sizeof(qd)) > 0)
          {
              if ((a = malloc(strlen(p) + qd.d_szname + 1)) == NULL)
              {
                free((voidp *)p);
                io_close(chid);
                return ZE_MEM;
...
#endif
             SSTAT(name, &s)) != 0
#ifdef _DCC         // we don't want directories
        || (s.st_mode & S_IFDIR) != 0
#endif
    )
...
#ifdef QDOS
    /* source and destination on the same device */
    if (strncmp(s, d, 5) == 0)
        rename(s, d);    /* just rename the file */
    else                 /* copy file and delete the source */                  
#else
    if(rename(s, d))
#endif /* !QDOS */
    {
        ...
        if(fcopy(f, g, -1U) != ZE_OK)
        {
            fprintf(stderr, "error copying destination file %s -> %s\n",
                s, d);
            return(ZE_CREAT);
        }
        fclose(f);
...
#if !defined(VMS) && !defined(ATARI_ST) && !defined (_DCC)
...
{
#if defined (VMS) || defined(MACOS) || defined (_DCC)
  return 0;
#else /* !VMS */
  return chmod(f, a);
#endif /* ?VMS */

...
typedef qdirect_t qdirect, *qdirptr;
typedef struct qxtra
{
...
#ifndef UTIL
   int percent OF((long, long));
   int zipup OF((struct zlist far *, FILE *));
   int file_read OF((char *buf, unsigned size));
#endif /* !UTIL */
#ifdef QDOS
   long rev_long OF((long l));
   short rev_short OF((short s));
#endif /* QDOS */
...
#  else /* !UTIL */
     ...
     extern char *key;
#    define zputc(c,f) (putc(key!=NULL? zencode(c) : (c),(f)))
#  endif /* ?UTIL */
#endif /* ?CRYPT */

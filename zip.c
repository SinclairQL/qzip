...
#ifdef QDOS
/* struct WINDOWDEF _condetails = { 2, 1, 0, 7, 500, 220, 2, 30 }; */
char _prog_name[] = "zip";
...
#endif /* !S_IFLNK */
#ifdef QDOS
"  -H   include QDOS header",
#endif
"  -x   exclude these names"
  };
...
/* Do command line expansion for MSDOS, QDOS and VMS */
#if defined(MSVMS) && !defined(__GO32__) || defined(QDOS)
   ...
   command help in help() above. */
{
...

  {
    if (*p == ':')
      p++;
#elif QDOS
  if (tempath == NULL && (p = strchr(zipfile, '_')) != NULL)
  {
#else /* ?QDOS */
  if (tempath == NULL && (p = strrchr(zipfile, '/')) != NULL)
  {

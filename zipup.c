   ...
   ZE_ class.  Also, update tempzn by the number of bytes written. */
{
  ...
  else
  {
   ...
   if (vms_native)
    ...
    if(headput)
    {
        qdosextra       *local_qe, *central_qe;

        local_qe    = (qdosextra *)malloc(sizeof(qdosextra));
        central_qe  = (qdosextra *)malloc(sizeof(qdosextra));
        if(local_qe && central_qe)
        {
            if(0 == qstat((*z).name, &(local_qe->header)))
            {
                local_qe->shortid = rev_short(SHORTID);
                local_qe->len.lo  = (unsigned char)(EXTRALEN & 0xff);
                local_qe->len.hi  = (unsigned char)(EXTRALEN >> 8);
                strcpy(local_qe->longid, LONGID);

                memcpy(central_qe, local_qe, sizeof(qdosextra));

                z->ext      =   sizeof(qdosextra);
                z->cext     =   sizeof(qdosextra);
                z->extra    =   (char *)local_qe;
                z->cextra   =   (char *)central_qe;
            }
            else
                fprintf(stderr, "%s: warning: cannot stat %s, no file header added\n", "zip", z->name);
        }
        else
            fprintf(stderr, "%s: warning: out of memory %s, no file header added\n", "zip", z->name);
    }
#endif /* QL ZIP */
  ...
  z->vem = dosify ? 20 :                /* Made under MSDOS by PKZIP 2.0 */
                    ...
                    0x100 + REVISION;   /* Made under Amiga by this Zip */
#else
# ifdef VMS
...
#   elif QDOS
                    0xc00 + REVISION;   /* Made under QDOS by this Zip */ 
#   else /* !MSDOS */
